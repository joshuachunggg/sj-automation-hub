...
<script>let id='',reviewing=false;const v=x=>document.getElementById(x).value,esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),cls=s=>String(s||'pending').toLowerCase().replaceAll(' ','-');async function openEditor(url){await fetch('/api/open-editor',{method:'POST',body:JSON.stringify({url})})}function card(x,extra=''){return '<div class="card '+cls(x.status)+' '+extra+'"><b>'+esc(x.site||'Worker')+'</b><small>'+esc(x.status||'pending')+'</small>'+(x.link?'<br><button class="alt" onclick="openEditor(\''+esc(x.link)+'\')">Open AEM editor</button>':'')+'</div>'}function draw(d){let el=document.getElementById('dashboard');if(!d.kind){el.innerHTML='<small>Waiting for automation progress.</small>';return}if(d.kind==='publish'){let locales=Object.values(d.locales),done=locales.filter(x=>/live$|error/.test(x.status)).length;if(d.mode.startsWith('validate'))locales=locales.map(x=>({...x,status:x.status==='live'?'validated':x.status}));el.innerHTML='<h2>Live publishing · '+done+'/'+(d.total||locales.length)+'</h2><h3>Workers</h3><div class="grid">'+Object.entries(d.workers).map(([n,x])=>card({site:'Worker '+n+(x.site?' · '+x.site:''),status:x.status},'worker')).join('')+'</div><h3>Countries</h3><div class="grid">'+locales.map(card).join('')+'</div>';return}let parents=Object.values(d.parents),children=Object.values(d.children),finished=parents.filter(x=>x.status!=='auditing').length,current=d.current||{};el.innerHTML='<h2>Finalize authoring · '+finished+'/'+(d.total||parents.length)+' audited</h2>'+(current.site?'<section><h3>Current QA · '+esc(current.site)+'</h3><button onclick="openEditor(\''+esc(current.link)+'\')">Open AEM editor</button>'+(current.findings?.length?'<h3>Heuristic differentials</h3><ul class="findings">'+current.findings.map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul>':'<p>No heuristic differentials found.</p>')+'</section>':'')+'<h3>QA status</h3><div class="grid">'+parents.map(card).join('')+'</div><h3>Child copies · '+children.filter(x=>x.status==='copied').length+'/'+(d.children_total||children.length)+'</h3><div class="grid">'+children.map(card).join('')+'</div>'}async function pick(field){let r=await fetch('/api/pick-workbook',{method:'POST',body:'{}'}),j=await r.json();if(j.path)document.getElementById(field).value=j.path}async function start(action,data={}){let r=await fetch('/api/start',{method:'POST',body:JSON.stringify({action,...data})}),j=await r.json();if(j.error)return alert(j.error);id=j.id;poll()}async function send(value){if(!id)return alert('Start sign in first.');await fetch('/api/job/'+id+'/input',{method:'POST',body:JSON.stringify({value})})}document.addEventListener('keydown',e=>{if(reviewing&&!e.metaKey&&!e.ctrlKey&&['y','n'].includes(e.key.toLowerCase()))send(e.key.toLowerCase())});async function poll(){if(!id)return;let r=await fetch('/api/job/'+id),j=await r.json();document.getElementById('log').textContent=j.output||'';document.getElementById('state').textContent=j.running?'Running':'Finished'+(j.code?' · failed':'');document.getElementById('title').textContent=j.action==='qa'?'Finalize authoring':j.action==='publish'?'Live publishing':'Run dashboard';draw(j.dashboard);reviewing=!!j.review;document.getElementById('review').innerHTML=j.locked?'<section><b>Workbook locked</b><p>'+esc(j.locked)+'</p><button onclick="send(\'\')">I closed it — retry</button></section>':j.review?'<section><b>QA decision needed</b><p>'+esc(j.review)+'</p><button onclick="send(\'y\')">Approve (Y)</button><button class="alt" onclick="send(\'n\')">Skip (N)</button></section>':'';if(j.running)setTimeout(poll,800)}</script>'''
